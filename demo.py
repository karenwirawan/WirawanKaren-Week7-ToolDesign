...
def demonstrate_success_and_error() -> None:
    """
    Demonstrate both a successful case (valid input) and an error case (invalid input).

    This function satisfies the requirement that demo.py show both in the same file:
    first a valid call that returns keywords, then an invalid call that is caught and
    handled without crashing.
    """
    print("=" * 60)
    print("SUCCESS vs ERROR – Same file (valid and invalid input)")
    print("=" * 60)

    # Successful case: valid input
    valid_text = "Artificial intelligence and machine learning are transforming technology."
    print("\n[SUCCESS] Valid input:")
    result = keyword_extractor.execute(text=valid_text, top_n=5)
    print(f"  Input: {valid_text[:50]}...")
    print(f"  Keywords: {[k['word'] for k in result['keywords']]}")

    # Error case: invalid input (empty text)
    print("\n[ERROR] Invalid input (empty text):")
    try:
        keyword_extractor.execute(text="", top_n=5)
...
def main() -> None:
    ...
    Runs (all in this file):
        - success + error pair (valid vs invalid input)
        - more successful extraction examples
        - more invalid input / error handling examples
        - a simple agent-style workflow (success and error)
    """
    print("\nKeyword Extractor Tool – Demo\n")
    demonstrate_success_and_error()
